...
import { Bell, Calendar, FileText, Video, User, Package, MessageSquare, PlusCircle, CheckCircle, XCircle, RefreshCw } from 'lucide-react'; // Importer RefreshCw
import axios from 'axios';
import "./styles/NotificationsPage.css";
import { useUnreadCount } from './contexts/UnreadCountContext';

const NotificationsDocteurPage = () => {
...
const NotificationItem = ({ notification, onMarkAsRead }) => {
  const {
    ...
    doctor_name,
    patient_name,
    event_type, // Pour les notifications de service
    notifier_name, // Pour les notifications de service (qui a déclenche)
    patient_name: updated_patient_name, // Pour les mises à jour de profil patient
  } = notification.data;
  ...
  const getIcon = () => {
    switch(type) {
      ...
      case 'schedule_event': // Nouveau type pour les événements d'emploi du temps
        switch(event_type) {
          case 'nouvelle_garde': return <Calendar size={16} />; // Ou une icône spécifique pour une nouvelle garde
  ...
  if (type === 'rendez_vous') {
    ...
    titreNotification = notification.data.title;
  } else if (type === 'schedule_event') { // Nouveau: pour les notifications d'emploi du temps
  ...
  return (
    <div className={`soinlink-notification-item ${!notification.read_at ? 'soinlink-unread' : ''}`}>
      ...
      <div className="soinlink-notification-content">
        ...
        <p className="soinlink-notification-message">{message}</p>
...
export default NotificationsDocteurPage;
